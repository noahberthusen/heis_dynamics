...
if __name__ == "__main__":
    ...
    def VTD(tf, dt, p, init):
        ...
        if (os.path.exists(f'./results_{L}/zVTD_results_{tf}_{L}_{p}_{dt}.csv')):
            VTDStepList = pd.read_csv(f'./results_{L}/zVTD_results_{tf}_{L}_{p}_{dt}.csv', index_col=0)
        ...
        init_params = np.random.uniform(0, np.pi, 2*L*p)
        ...
        VTDStepList.to_csv(f'./results_{L}/zVTD_results_{tf}_{L}_{p}_{dt}.csv')
